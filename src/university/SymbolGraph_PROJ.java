 ...
 *  @author Kevin Wayne
 */
public class SymbolGraph_PROJ implements Serializable {
    private ST<String, Integer> st;  // string -> index
    private String[] keys;           // index  -> string
    private EdgeWeightedDigraph_PROJ wDiGraph;             // the underlying graph

    public SymbolGraph_PROJ(ST<String, Integer> st, String[] keys, EdgeWeightedDigraph_PROJ wDiGraph) {
        this.st = st;
        this.keys = keys;
        this.wDiGraph = wDiGraph;
    }
    ...
    public SymbolGraph_PROJ(String filename, String delimiter) {
        ...
        // line to all others
        wDiGraph = new EdgeWeightedDigraph_PROJ(st.size());
        in = new In(filename);
        while (in.hasNextLine()) {
            String[] a = in.readLine().split(delimiter);
            int v = st.get(a[0]);
            int w = st.get(a[1]);
            wDiGraph.addEdge(new DirectedEdge_PROJ(v, w, Double.parseDouble(a[2]), Double.parseDouble(a[3])));
            /*for (int i = 1; i < a.length; i++) {
                int w = st.get(a[i]);
                wGraph.addEdge(new Edge(v, w,Double.parseDouble(a[i+1])));
            }*/
        }
    }
     ...
     */
    @Deprecated
    public EdgeWeightedDigraph_PROJ G() {
        return wDiGraph;
    }
     ...
     * @return the graph associated with the symbol graph
     */
    public EdgeWeightedDigraph_PROJ graph() {
        return wDiGraph;
    }
    ...
    public static void main(String[] args) {
        ...
        SymbolGraph_PROJ sg = new SymbolGraph_PROJ(filename, delimiter);
        EdgeWeightedDigraph_PROJ graph = sg.graph();
        while (StdIn.hasNextLine()) {
            String source = StdIn.readLine();
            if (sg.contains(source)) {
                int s = sg.index(source);
                for (DirectedEdge_PROJ v : graph.adj(s)) {
                    StdOut.println("   " + sg.name(v.to()));
                }
            }
            else {
    ...
    public EdgeWeightedDigraph_PROJ getwDiGraph() {
        return wDiGraph;
    }

    public void setwDiGraph(EdgeWeightedDigraph_PROJ wDiGraph) {
        this.wDiGraph = wDiGraph;
    }
}
