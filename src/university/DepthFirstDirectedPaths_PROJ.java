 ...
 *  @author Kevin Wayne
 */
public class DepthFirstDirectedPaths_PROJ {
     ...
     */
    public DepthFirstDirectedPaths_PROJ(EdgeWeightedDigraph_PROJ G, int s) {
    ...
    private void dfs(EdgeWeightedDigraph_PROJ G, int v) {
        marked[v] = true;
        for (DirectedEdge_PROJ w : G.adj(v)) {
            if(!w.isDisabled()) {
                if (!marked[w.to()]) {
                    edgeTo[w.to()] = v;
                    dfs(G, w.to());
